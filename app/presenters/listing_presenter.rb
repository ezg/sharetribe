class ListingPresenter < MemoisticPresenter
  ...
  def delivery_config(require_shipping_address, pickup_enabled, shipping_price, shipping_price_additional, currency)
    ...
    when matches([true, true])
      shipping = delivery_price_hash(:shipping, shipping_price, shipping_price_additional, true) if require_shipping_address
      pickup = delivery_price_hash(:pickup, Money.new(0, currency), Money.new(0, currency), false)
      [shipping, pickup]
    when matches([true, false])
      shipping = delivery_price_hash(:shipping, shipping_price, shipping_price_additional, true) if require_shipping_address
      [shipping]
    when matches([false, true])
      pickup = delivery_price_hash(:pickup, Money.new(0, currency), Money.new(0, currency), true)
      [pickup]
    else
      []
    end
  ...
  def delivery_price_hash(delivery_type, price, shipping_price_additional, d)
    { name: delivery_type,
      price: price,
