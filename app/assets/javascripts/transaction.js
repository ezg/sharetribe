...
(function (module, _) {
  ...
  function initializePayPalBuyForm(formId, analyticsEvent) {
    ...
    var formSubmitWithData = $form
      .asEventStream("submit", function (ev) {
        ev.preventDefault();
    ...
    var analyticsEventSent = formSubmitWithData.flatMapLatest(function () {
    ...
    submitInProgress
      .skipDuplicates()
      .onValue(_.partial(toggleSpinner, $spinner));
  ...
  function initializeCreatePaymentPoller(opStatusUrl, redirectUrl) {
    ST.utils
      .baconStreamFromAjaxPolling({ url: opStatusUrl }, function (
        pollingResult
      ) {
        return pollingResult.completed;
      })
