module PcpHelper
  ...
  def community_ready_for_payments?(community_id)
    #pcp_active?(community_id) &&
    #  Maybe(TxApi.settings.get(community_id: community_id, payment_gateway: :pcp, payment_process: :preauthorize))
    #  .map {|res| res[:success] ? res[:data] : nil}
    #  .select {|set| set[:commission_from_seller] && set[:minimum_price_cents]}
    #  .map {|_| true}
    #  .or_else(false)
    return pcp_active?(community_id)
  end

  def pcp_active?(community_id)
    settings = Maybe(TxApi.settings.get(community_id: community_id, payment_gateway: :pcp, payment_process: :preauthorize))
  ...
  def pcp_provisioned?(community_id)
  ...
  def user_and_community_ready_for_payments?(person_id, community_id)
    pcp_active?(community_id) && user_pcp_active?(community_id, person_id)
  end
